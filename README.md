...
| **扫描选项** | 解析器勾选、通用扫描、去重、激进模式、最短/最长长度、**只保留中文行**、**封包覆盖**、**跟踪说话人声明**、**自动识别单独成行的角色名**、**旁白标记**、PDF 字体 |
...
| `--narration-label` | 空 | 给旁白补的说话人名字（如 `旁白`），让每行都有名字 |
| `--guess-speakers` | 关 | 把单独成行的短行当成角色名（启发式） |
| `--no-speaker-tracking` | — | 不跨行跟踪 `[name text=…]` 这类说话人声明 |
...
galgame 脚本有三种常见写法，工具都支持：

| 写法 | 例子 | 默认 |
| --- | --- | --- |
| 名字和台词同一行 | `悠斗「早上好」` | ✅ 直接识别 |
| 名字用标签**单独声明** | `[name text="悠斗"]` ⏎ `早上好` | ✅ 会一直沿用到下一个声明 |
| 名字**单独成行** | `悠斗` ⏎ `早上好` | ⚠️ 需打开「自动识别单独成行的角色名」 |

第三种默认关闭：`翌日` 这种短旁白和角色名长得一模一样，打开后可能误判，
所以先确认效果再用。

**旁白本身没有说话人。** 想让每行都带名字，在「旁白标记」里填 `旁白`
（CLI：`--narration-label 旁白`），旁白就会署名 ——
txt / CSV / JSON / 剧本 PDF 全都会带上。
