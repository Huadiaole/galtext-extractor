...
galgame 脚本声明人名有三种写法，这一版全都支持：

| 写法 | 长这样 | 默认 |
| --- | --- | --- |
| 名字和台词同一行 | `悠斗「早上好」` | ✅ |
| 名字用标签单独声明 | `[name text="悠斗"]` ⏎ `早上好` | ✅ 会沿用到下一个人名 |
| 名字单独成行 | `悠斗` ⏎ `早上好` | ⚠️ 需开「自动识别单独成行的角色名」 |

第二种是最容易出问题的：声明行本质是个标签，**很容易被当作垃圾整行丢掉**，
结果后面一大片台词全都没有名字。这一版专门修了它。

旁白本来就没有说话人。想让**每行都带名字**，在「旁白标记」里填 `旁白`
（CLI：`--narration-label 旁白`），旁白就会署名，所有导出格式都会带上。

另外，扫描结束时会提示「有多少条文本含假名」—— 残留的日文原文有多少、占几成，
一眼就能看到，不用自己数。
